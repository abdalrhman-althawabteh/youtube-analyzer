...
import { ChannelData, AIAnalysis, OnboardingData, ContentIdea, StrategyStep } from '../types';
import StatCard from './StatCard';
import { generateMoreIdeas, regenerateStrategy } from '../services/geminiService';
import { supabase } from '../services/supabaseClient';
import { useNavigate } from 'react-router-dom';
import {
  Users, Eye, Video, Activity, Zap, Star, LogOut,
  ExternalLink, Calendar, LayoutDashboard, Lightbulb, TrendingUp, MessageCircle, RefreshCw, PlusCircle
} from 'lucide-react';
import {
...
const Dashboard: React.FC<DashboardProps> = ({ channelData, analysis, userData }) => {
  ...
  const [contentIdeas, setContentIdeas] = useState<ContentIdea[]>(analysis.contentIdeas);
  const [strategy, setStrategy] = useState<StrategyStep[]>(analysis.strategyPlan);
  ...
  const handleGenerateMoreIdeas = async () => {
    ...
    const existingTitles = contentIdeas.map(i => i.title);
    const newIdeas = await generateMoreIdeas(userData, channelData, existingTitles, userData.geminiApiKey);
    setContentIdeas(prev => [...prev, ...newIdeas]);
    setLoadingIdeas(false);
  };

  const handleRegenerateStrategy = async () => {
    setLoadingStrategy(true);
    const newStrategy = await regenerateStrategy(userData, channelData, userData.geminiApiKey);
            ...
            {analysis.actionItems.map((item, idx) => (
              <div key={idx} className="p-3 bg-[#0F0F0F] rounded-xl border border-white/5 hover:border-white/10 transition-colors">
                <div className="flex justify-between items-center mb-1">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${item.priority === 'High' ? 'bg-red-500/20 text-red-500' :
    ...
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-gray-400 text-sm">أفكار تم توليدها بناءً على شخصية قناتك "{userData.passionNiche}".</p>
        </div>
        <button
          onClick={handleGenerateMoreIdeas}
          disabled={loadingIdeas}
          className="bg-white text-black px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-gray-200 transition-colors disabled:opacity-50"
        >
          {loadingIdeas ? <RefreshCw className="animate-spin" size={18} /> : <PlusCircle size={18} />}
          {loadingIdeas ? 'جاري التوليد...' : 'توليد أفكار جديدة'}
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {contentIdeas.map((idea, idx) => (
          <div key={idx} className="bg-[#1A1A1A] p-6 rounded-2xl border border-white/5 hover:border-[#FF0000] hover:shadow-lg hover:shadow-[#FF0000]/10 transition-all group flex flex-col h-full animate-fade-in-up">
            <div className="flex justify-between items-start mb-4">
              <span className={`px-2 py-1 rounded text-xs font-bold ${idea.format === 'Video' ? 'bg-red-500/10 text-red-500' :
  ...
  const renderStrategy = () => (
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">استراتيجية النمو (Dynamic Roadmap)</h2>
          <p className="text-gray-400 text-sm">خطة مصممة خصيصاً لمستوى {userData.creatorLevel} وهدف {userData.primaryGoal}.</p>
        </div>
        <button
          onClick={handleRegenerateStrategy}
          disabled={loadingStrategy}
          className="bg-[#272727] text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-[#333] transition-colors disabled:opacity-50"
        >
          {loadingStrategy ? <RefreshCw className="animate-spin" size={18} /> : <RefreshCw size={18} />}
          تحديث الاستراتيجية
        </button>
      </div>
      ...
      <div className="relative border-r-2 border-[#272727] mr-4 space-y-12 py-4">
        {strategy?.map((step, idx) => (
          <div key={idx} className="relative pr-8 animate-fade-in-left" style={{ animationDelay: `${idx * 100}ms` }}>
            {/* Timeline Dot */}
            <div className="absolute -right-[9px] top-0 w-4 h-4 rounded-full bg-[#FF0000] border-4 border-[#0F0F0F]"></div>

            <div className="bg-[#1A1A1A] p-6 rounded-2xl border border-white/5 hover:border-[#FF0000]/30 transition-all hover:bg-[#1A1A1A]/80">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 gap-2">
                <div>
                  <h3 className="text-xl font-bold text-white">{step.phase}</h3>
                  <span className="text-[#F39C12] text-sm font-bold bg-[#F39C12]/10 px-2 py-0.5 rounded ml-2">{step.timeline}</span>
                </div>
                <div className="bg-[#272727] px-4 py-2 rounded-lg text-center min-w-[100px]">
                  <div className="text-gray-500 text-[10px] uppercase tracking-wider">التركيز</div>
              ...
              <div className="space-y-3">
                {step.actions.map((action, i) => (
                  <div key={i} className="flex items-start gap-3 group">
                    <div className="bg-[#FF0000]/10 text-[#FF0000] w-6 h-6 rounded flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 transition-colors group-hover:bg-[#FF0000] group-hover:text-white">
                      {i + 1}
                    </div>
        ...
        {(!strategy || strategy.length === 0) && (
          <div className="text-center text-gray-500">لا توجد خطة متاحة حالياً. اضغط على تحديث الاستراتيجية.</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex h-screen bg-[#0F0F0F] overflow-hidden font-sans" dir="rtl">
      {/* Sidebar */}
      <aside className="w-64 bg-[#0F0F0F] border-l border-white/5 hidden md:flex flex-col">
        <div className="p-6">
          <h1 className="text-2xl font-black text-white tracking-tighter cursor-default">
        ...
        <nav className="flex-1 px-4 space-y-2">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-gray-400 hover:text-red-500 hover:bg-red-500/10 transition-all mt-4"
          >
            <LogOut size={20} />
